...
AgentForge不仅是代码框架，更是一套面向业务开发的AI助手协作规范。它通过根目录规则和项目级说明，让主流AI编程助手理解业务目标、开发约束和可接受的修改范围。

### 目标

- 让 AI 助手专注于业务功能实现，而不是框架决策
- 为企业级开发保留可维护性、类型安全和测试原则
- 提供一致的助手规则，避免不同工具给出冲突建议
...
1. **定义业务目标**
   - 先用文档或 issue 说明场景：客服、订单、知识检索、自动化流程等
   - 明确结果：API、前端页面、工具接口、数据流
2. **生成基础模板**
   - 通过 `agentforge create` 创建项目骨架
   - 在 `src/agents`、`src/tools`、`apps/web`、`apps/api` 中补齐业务结构
3. **使用 AI 助手补全逻辑**
   - 让 Copilot/Claude/Cursor 等助手读取根目录规则
   - 让 AI 识别项目风格：TypeScript 严格模式、事件驱动 Agent、Zod 验证
4. **验证与测试**
   - 使用测试用例验证业务流程
   - 保持改动范围最小，仅实现当前业务需求
...
- `CLAUDE.md` / `AGENTS.md`
  - 根目录主文档，供 Claude Code、Cursor、Codex、Windsurf 等助手读取
- `.github/copilot-instructions.md`
  - GitHub Copilot 的仓库级提示说明
- `.windsurf/rules/agentforge.md`
  - Windsurf Cascade 规则文件
...
- 优先实现“可交付功能”，而非通用框架抽象
- 用 `src/tools` 封装外部服务调用与业务接口
- 用 `src/agents` 建立业务流程和工具调用路径
- 用 `apps/web` 展示结果、用 `apps/api` 提供稳定 API
- 将 AI 助手生成的内容纳入代码审查，而不是直接接受所有修改

### Karpathy 四原则在 AgentForge 中的落地
...
AgentForge 的 AI 协作设计鼓励团队持续迭代：

- 把规则文档作为团队编码约定
- 在项目中补充业务约束和约定
- 定期更新 `CLAUDE.md` / `AGENTS.md` 等规则文件，保持与实际项目状态一致

以上内容构成了 AgentForge 对企业级 TypeScript 全栈业务开发与 AI 助手协作的核心说明。
